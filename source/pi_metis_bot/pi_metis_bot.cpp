...
telegram_bot::telegram_bot(const std::string &token) 
    : token(token), image_path("/home/yaba/Sandbox/PiMetis/source/pi_metis_ia/person.jpg")
{
...
telegram_bot::telegram_bot(const std::string &token, const std::string& chat_id) 
    : token(token), chat_id(chat_id), image_path("/home/yaba/Sandbox/PiMetis/source/pi_metis_ia/person.jpg")
{
    this->url = this->url + token;
